...
def display_evaluation(validate_data_loaders,model,epoch,mode):
    ...
    for data_loader in validate_data_loaders:
        ...
        for i, [data_ev,mask] in enumerate(data_loader):  ## data_ev:(batch_size, 2, length)
            ...
            iter += 1
    if mode=='val':
        with open('/home/sh_jun/Bi_LSTM_CRF_test/intermediate/validae_res.pkl', 'wb') as f:
            pickle.dump(results, f)
            f.close()
    elif mode=='train':
        with open('/home/sh_jun/Bi_LSTM_CRF_test/intermediate/train_res.pkl', 'wb') as f:
            pickle.dump(results, f)
            f.close()
    else:
        with open('/home/sh_jun/Bi_LSTM_CRF_test/intermediate/test_res.pkl', 'wb') as f:
            pickle.dump(results, f)
            f.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint', default='./checkpoint/doc', help='path to checkpoint prefix')
    ...
    print("test training data start")
