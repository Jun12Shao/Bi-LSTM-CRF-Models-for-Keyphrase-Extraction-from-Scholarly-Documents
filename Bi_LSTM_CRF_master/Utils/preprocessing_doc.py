...
def get_Vocabulary(data_tr,data_val):
    ...
    with open('./intermediate/dictionary.pkl', 'wb') as f:
...
def get_Embedding(id2word):
    ...
    with open('./intermediate/Embedding.pkl', 'wb') as f:
        pickle.dump(emb, f)
        f.close()
...
def cleandata(fileabs, filekey):
    ...
    contents={}
    for document in datab:
        cleanAbs=[]
        for line in document:
            cleanAbs+=list(filter(filter_function,re.split('[^a-zA-Z0-9]',line)))
        src_index=cleanAbs.index('src')
        id2=int(cleanAbs[src_index-1])
        contents[id2]=cleanAbs[src_index+1:]
...
def Generate_sequence(data,word2id,thresholds, name):
    ...
    buck_index=np.ones(len(data),dtype=int)*3
    ...
    for n, document in tqdm(enumerate(data)):
        N=len(document[0])
        idx=buck_index[n]
        pointer[idx]+=1
        # try:
        ...
        for keys in document[1]:
            ...
            for word in keys:
                try:
                    ...
                    unmatch+=1
            if not (unmatch>0 and len(key_)-unmatch<=2):    ## here is a kind of approximation for keyword unseen in vocabulary
                key_=np.array(key_)
                keyphrases.append(key_)

        ## generate ground true target label.
        # targets=sequence=np.zeros(thresholds[idx]+2,dtype=int)

        for keyphrase in keyphrases:
            index=locate_target(buckets[idx][pointer[idx]-1,0],keyphrase)
            for id in index:
                buckets[idx][pointer[idx]-1,1,id:id+len(keyphrase)]=1
        buckets[idx][pointer[idx]-1,1,new_length+1:-1]=tag_2_id['pad']


    ## Save emb as Embedding.pkl
    with open('./intermediate/'+name, 'wb') as f:
...
with open('./intermediate/Embedding.pkl', 'rb') as f:
